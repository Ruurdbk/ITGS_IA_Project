...
Sprint 5: Implementation of CLI
@author: Ruurd
"""
#Import libraries
...
parser = argparse.ArgumentParser(description='This is a conversion script for NED soundings to XYZ')
parser.add_argument('-i','--input', help='Input NED file directory', required=False)
parser.add_argument('-o','--output', help='Output XYZ file directory', required=False)
parser.add_argument('-s','--statistics', help='Run conversion with statistics', required=False)
parser.add_argument('-m','--metadata', help='Run conversion with logging of metadata', required=False)
...
print(out_path)    
if args.statistics:
    efficiency_statistics = args.statistics

if args.metadata:
    metadata_loggin = args.metadata
...
except:   #When no directory is found, print and exit
    print("Directory not found")
    sys.exit(1)
    
#Print and exit if no NED file is found in directory
if len(nedfiles) == 0:
    print("No NED files in directory", in_path)
    sys.exit(2)
...
for nedfile in nedfiles:
    ...
    except:   #If file is corrupt, log and start conversion of next file
